"""
Sentinel AI — MySQL Data Connector

Provides SQL database schema discovery, table inspection, connection health testing,
and data streaming for MySQL / MariaDB relational databases.
"""
...
class MySQLConnector(BaseConnector):
    """
    Data connector for MySQL relational databases.
    ...
    def connect(self) -> None:
        ...
        if errors:
            raise ConfigurationError("MySQL Connector validation failed", errors=errors)
        ...
        try:
            ...
            raise ConnectionError(f"Failed to connect to MySQL at '{self.host}:{self.port}': {str(e)}")
    ...
    def list_tables(self) -> list[str]:
        ...
        try:
            ...
            raise SchemaDiscoveryError(f"Failed to list MySQL tables: {str(e)}")
    ...
    def fetch_schema(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise SchemaDiscoveryError(f"Failed to fetch schema for MySQL table '{self.table_name}': {str(e)}")
    ...
    def preview(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ReadError(f"Failed to preview MySQL table '{self.table_name}': {str(e)}")
    ...
    def read(
        ...
        try:
            ...
            raise ReadError(f"Failed to read MySQL table '{self.table_name}': {str(e)}")
    ...
    def get_metadata(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.table_name:
                with self._engine.connect() as conn:
                    res = conn.execute(text(f"SELECT COUNT(*) FROM `{self.table_name}`"))
            ...
            raise ReadError(f"Failed to extract MySQL metadata: {str(e)}")

    def health_check(self) -> dict[str, Any]:
        """Perform health check on the MySQL endpoint."""
